...
function BreakdownTable({ heading, empty, hoursLabel, rows, onOpen }: { readonly heading: string; readonly empty: string; readonly hoursLabel: string; readonly rows: readonly { readonly key: string; readonly label: string; readonly hours: number }[]; readonly onOpen?: (key: string) => void }) {
  return <section className="actual-breakdown"><h5>{heading}</h5>{rows.length === 0 ? <p className="empty-copy">{empty}</p> : <table><thead><tr><th scope="col">{heading}</th><th scope="col">{hoursLabel}</th></tr></thead><tbody>{rows.map((row) => <tr key={row.key}><th scope="row">{onOpen === undefined ? row.label : <button className="text-link" onClick={() => onOpen(row.key)} type="button">{row.label}</button>}</th><td>{row.hours}</td></tr>)}</tbody></table>}</section>;
}
