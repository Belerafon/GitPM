    ...
    <div className="project-files-panel">
        ...
        <ul>{queue.map((item) => {
          ...
          return <li className={`project-files-upload-item is-${item.state}`} data-upload-id={item.id} key={item.id} tabIndex={-1}>
            ...
            {item.state === "conflict" && <div className="project-files-conflict">
              ...
              <label data-field-hint={t("fieldHint.projectFileName")}>{t("projectFiles.otherName")}<input aria-label={t("projectFiles.otherNameFor", { name: item.name })} defaultValue={item.name} key={`${item.id}:${item.name}`} /></label>
      ...
      {selected !== undefined && fileAction !== undefined && <div aria-labelledby="project-file-action-title" aria-modal="true" className="project-file-large-dialog" onKeyDown={onActionKeyDown} onMouseDown={(event) => { if (event.target === event.currentTarget && !actionBusy) closeAction(); }} ref={actionDialog} role="dialog">
        <div className="project-file-large-card project-file-action-card">
          ...
          {fileAction === "rename" && <>
            <label data-field-hint={t("fieldHint.projectFileName")}>{t("projectFiles.newName")}<input autoFocus disabled={actionBusy} onChange={(event) => setActionValue(event.target.value)} value={actionValue} /></label>
            ...
            {referencePreview?.status === "ready" && <div className="project-file-reference-consequences">
              ...
              <label data-field-hint={t("fieldHint.projectFileReferenceMode")}><input checked={referenceChoice === "update"} name="rename-reference-mode" onChange={() => setReferenceChoice("update")} type="radio" />{t("projectFiles.renameUpdateReferences", { count: referencePreview.value.count })}</label>
              <label data-field-hint={t("fieldHint.projectFileReferenceMode")}><input checked={referenceChoice === "keep"} name="rename-reference-mode" onChange={() => setReferenceChoice("keep")} type="radio" />{t("projectFiles.renameKeepReferences", { count: referencePreview.value.count })}</label>
            </div>}
          </>}
          {fileAction === "replace" && <>
            {referencePreview?.status === "loading" && <p role="status">{t("projectFiles.referencesChecking")}</p>}
            ...
            <label data-field-hint={t("fieldHint.projectFileReplacement")}>{t("projectFiles.replaceSelectFile")}<input autoFocus disabled={actionBusy} onChange={(event) => { setReplacementFile(event.currentTarget.files?.[0]); setReplacementConfirmation(""); setReplacementProgress(0); }} type="file" /></label>
            {replacementFile !== undefined && <>
              ...
              {replacementFile.size > PROJECT_FILE_LARGE_WARNING_BYTES && <label data-field-hint={t("fieldHint.projectFileConfirmation")}>{t("projectFiles.largeTypeName", { name: replacementFile.name })}<input disabled={actionBusy} onChange={(event) => setReplacementConfirmation(event.target.value)} value={replacementConfirmation} /></label>}
      ...
      {activeConfirmation !== undefined && <div aria-labelledby="project-file-large-title" aria-modal="true" className="project-file-large-dialog" onKeyDown={(event) => { if (event.key === "Escape") cancelItem(activeConfirmation); }} role="dialog">
        <div className="project-file-large-card">
          ...
          <label data-field-hint={t("fieldHint.projectFileConfirmation")}>{t("projectFiles.largeTypeName", { name: activeConfirmation.name })}<input autoFocus onChange={(event) => setConfirmation(event.target.value)} value={confirmation} /></label>
