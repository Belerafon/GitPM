...
function Shell({ locale, setLocale, api, navigate, confirmAction }: {
  ...
  return (
    ...
    >
        ...
        {view === "nav.board" && (active === undefined ? <div className="card empty-workspace">{t("core.selectProject")}</div> : <BoardWorkspace api={api} draft={active} key={`nav.board:${workspaceSelection.projectId ?? ""}:${workspaceSelection.query?.status?.[0] ?? ""}:${workspaceSelection.query?.type?.[0] ?? ""}:${workspaceSelection.query?.view?.[0] ?? ""}`} locale={locale} initialProjectId={workspaceSelection.projectId} initialStatusFilter={workspaceSelection.query?.status?.[0]} initialTypeFilter={workspaceSelection.query?.type?.[0]} initialViewId={workspaceSelection.query?.view?.[0]} onNavigate={openWorkspace} onChanged={drafts.refresh} />)}
