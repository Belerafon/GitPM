...
class BrowserAcceptanceApi implements GitPmApi {
  ...
  private statusConfig: ConfigurationResult | undefined;
  private issueTypeConfig: ConfigurationResult | undefined;
  private pollCount = 0;
  ...
  async createDraft(draftId: string) {
    ...
    if (window.location.pathname.endsWith("vfy-026.html")) {
      ...
      this.entities = [
        this.entityResult({ schema: "gitpm/project@2", id: project, name: "Beta portfolio", status: "backlog", lifecycle: "active" }),
        this.entityResult({ schema: "gitpm/task@2", id: "T-26-222222", project, title: "Prepare beta", type: "task", status: "backlog", lifecycle: "active" }),
        this.entityResult({ schema: "gitpm/task@2", id: "T-26-333333", project, title: "Review Board", type: "task", status: "in-progress", lifecycle: "active" }),
        this.entityResult({ schema: "gitpm/task@2", id: "T-26-444444", project, title: "Alpha accepted", type: "task", status: "done", lifecycle: "active" }),
      ];
      this.changedFiles = [];
    }
    if (window.location.pathname.endsWith("vfy-027.html")) {
      ...
      this.entities = [
        this.entityResult({ schema: "gitpm/project@2", id: project, name: "Beta portfolio", status: "backlog", lifecycle: "active" }),
        this.entityResult({ schema: "gitpm/milestone@2", id: milestone, project, name: "Beta release", lifecycle: "active", schedules: { plan: { finish: "2026-07-08" } } }),
        this.entityResult({ schema: "gitpm/task@2", id: ids[0]!, project, title: "Plan release", type: "task", status: "done", lifecycle: "active", schedules: { plan: { start: "2026-07-01", finish: "2026-07-05" } } }),
        this.entityResult({ schema: "gitpm/task@2", id: ids[1]!, project, title: "Build API", type: "task", status: "done", lifecycle: "active", parent: ids[0], milestone, schedules: { plan: { start: "2026-07-02", finish: "2026-07-03" } } }),
        this.entityResult({ schema: "gitpm/task@2", id: ids[2]!, project, title: "Ship UI", type: "task", status: "in-progress", lifecycle: "active", schedules: { plan: { start: "2026-07-04", finish: "2026-07-06", depends_on: [ids[1]!] } } }),
        this.entityResult({ schema: "gitpm/task@2", id: ids[3]!, project, title: "Review", type: "task", status: "backlog", lifecycle: "active", schedules: { plan: { start: "2026-07-06", finish: "2026-07-07", depends_on: [ids[2]!] } } }),
        this.entityResult({ schema: "gitpm/task@2", id: ids[4]!, project, title: "Launch", type: "task", status: "backlog", lifecycle: "active", schedules: { plan: { start: "2026-07-08", finish: "2026-07-08", depends_on: [ids[3]!] } } }),
        this.entityResult({ schema: "gitpm/task@2", id: "T-26-777777", project, title: "Undated hidden", type: "task", status: "backlog", lifecycle: "active" }),
        this.entityResult({ schema: "gitpm/task@2", id: "T-26-999999", project, title: "Archived hidden", type: "task", status: "done", lifecycle: "archived", schedules: { plan: { start: "2026-07-01", finish: "2026-07-02" } } }),
      ];
      this.changedFiles = [];
    }
    if (window.location.pathname.endsWith("vfy-028.html") || window.location.pathname.endsWith("vfy-032.html")) {
      ...
      this.entities = [
        ...
        this.entityResult({ schema: "gitpm/task@2", id: "T-26-555555", project, title: "Shared delivery", type: "task", status: "in-progress", lifecycle: "active", schedules: { plan: { start: "2026-07-06", finish: "2026-07-10", effort_hours: 40 } }, assignees: [ada, linus] }),
        this.entityResult({ schema: "gitpm/task@2", id: "T-26-666666", project, title: "Ada follow-up", type: "task", status: "backlog", lifecycle: "active", schedules: { plan: { start: "2026-07-09", finish: "2026-07-15", effort_hours: 30 } }, assignees: [ada] }),
        this.entityResult({ schema: "gitpm/task@2", id: "T-26-777777", project, title: "Undated excluded", type: "task", status: "backlog", lifecycle: "active", schedules: { plan: { effort_hours: 10 } }, assignees: [ada] }),
        this.entityResult({ schema: "gitpm/task@2", id: "T-26-888888", project, title: "Archived excluded", type: "task", status: "done", lifecycle: "archived", schedules: { plan: { start: "2026-07-06", finish: "2026-07-10", effort_hours: 10 } }, assignees: [ada] }),
      ];
      this.changedFiles = [];
    }
    if (window.location.pathname.endsWith("vfy-029.html")) {
      ...
      this.entities = [
        this.entityResult({ schema: "gitpm/project@2", id: project, name: "Agent portfolio", status: "backlog", lifecycle: "active" }),
        this.entityResult({ schema: "gitpm/task@2", id: task, project, title: "Before agent write", type: "task", status: "backlog", lifecycle: "active", description_markdown: "Initial value" }),
      ];
  ...
  async snapshot(draftId: string): Promise<DraftSnapshot> {
    ...
    if (window.location.pathname.endsWith("vfy-029.html") && this.pollCount === 2) {
      const current = this.entities.find((item) => item.document.schema === "gitpm/task@2");
  ...
  async restoreEntity(draftId: string, entityType: string, entity: EntityResult, fingerprint: string): Promise<EntityResult> { return await this.updateEntity(draftId, entityType, entity, fingerprint, { ...entity.document, lifecycle: "active" }); }
  ...
  async getConfiguration(_draftId: string, kind: "statuses" | "issue-types" | "work-categories" | "schedule-tracks"): Promise<ConfigurationResult> {
    if (kind === "schedule-tracks") return { document: { schema: "gitpm/schedule-tracks@1", tracks: [{ slug: "plan", title: "Plan", kind: "manual", capabilities: ["dates", "effort", "dependencies"] }], defaults: { enabled_tracks: ["plan"], primary_track: "plan", workload_track: "plan", dashboard_tracks: ["plan"] } } as ConfigurationDocument, path: ".gitpm/schedule-tracks.yaml", blob_id: "e".repeat(40), draft_fingerprint: "d".repeat(64) };
  ...
  async listChanges(): Promise<ChangesList> { if (new URLSearchParams(window.location.search).get("git_error") === "1") throw new Error("Git diff недоступен: рабочее дерево изменилось"); return { files: this.changedFiles, changed_files_count: this.changedFiles.length, affected_projects: this.changedFiles.length === 0 ? [] : ["P-26-A1PHA1"], project_files: [] }; }
  ...
  async semanticChanges(): Promise<SemanticDiff> {
    ...
    const item = (id: string, path: string, field: string, before: unknown, after: unknown) => ({ id, path, schema: id === "P-26-A1PHA1" ? "gitpm/project@2" : "gitpm/task@2", project: "P-26-A1PHA1", fields: [{ field, before, after }] });
