...
import { DEFAULT_WORKING_CALENDAR, localCalendarDate } from "./vacation-calendar-model.js";
...
export function PeopleProfileWorkspace({ api, confirmAction = () => true, draft, locale, onChanged = async () => undefined, personId, role = "Reporter", onNavigate, today = localCalendarDate() }: { readonly api: GitPmApi; readonly confirmAction?: (message: string) => boolean; readonly draft: DraftStatus; readonly locale: Locale; readonly onChanged?: () => Promise<void>; readonly personId: string; readonly role?: GitPmRole; readonly onNavigate: WorkspaceNavigate; readonly today?: string }) {
  ...
  const updateVacationAllowance = async (extraDays: number, reason: string) => {
    if (person === undefined) return false;
    const { annual_vacation_extra_days: _extraDays, annual_vacation_extra_days_reason: _extraReason, ...unchanged } = person.document;
    return await updatePerson({ ...unchanged, ...(extraDays > 0 ? { annual_vacation_extra_days: extraDays, annual_vacation_extra_days_reason: reason } : {}) });
  };
...
function PeopleProfile({ archivePerson, createAvailability, data, deletePerson, editorOpen, locale, onCloseEditor, onEdit, personId, readOnly, restorePerson, savePerson, onNavigate, t, today, updateAvailability, updateVacationAllowance }: { readonly archivePerson: () => Promise<boolean>; readonly createAvailability: (document: GitPmDocument) => Promise<boolean>; readonly data: ProfileData; readonly deletePerson: () => Promise<boolean>; readonly editorOpen: boolean; readonly locale: Locale; readonly onCloseEditor: () => void; readonly onEdit: () => void; readonly personId: string; readonly readOnly: boolean; readonly restorePerson: () => Promise<boolean>; readonly savePerson: (document: GitPmDocument) => Promise<boolean>; readonly onNavigate: WorkspaceNavigate; readonly t: (key: MessageKey, values?: Readonly<Record<string, string | number>>) => string; readonly today: string; readonly updateAvailability: (event: EntityResult, document: GitPmDocument) => Promise<boolean>; readonly updateVacationAllowance: (extraDays: number, reason: string) => Promise<boolean> }) {
  ...
  return <>
    ...
    <TaskCalendar availabilityEvents={availabilityEvents} calendar={calendar} key={personId} locale={locale} onNavigate={onNavigate} projectNames={projectNames} tasks={assignedTasks} text={text} t={t} today={today} />

    <div className="people-profile-layout">
      <main className="people-profile-main">
        <PeopleAvailability calendar={calendar === undefined ? DEFAULT_WORKING_CALENDAR : { workingWeekdays: numbers(calendar.document, "working_weekdays").length === 0 ? DEFAULT_WORKING_CALENDAR.workingWeekdays : numbers(calendar.document, "working_weekdays"), holidays: strings(calendar.document, "holidays") }} events={availabilityEvents} extraDays={number(person.document, "annual_vacation_extra_days")} extraDaysReason={text(person.document, "annual_vacation_extra_days_reason")} locale={locale} onCreate={createAvailability} onUpdate={updateAvailability} onUpdateAllowance={updateVacationAllowance} personId={personId} readOnly={readOnly} t={t} />
...
const initialTaskMonth = (tasks: readonly EntityResult[], text: TaskFieldReader, today: string) => {
...
function TaskCalendar({ availabilityEvents, tasks, calendar, projectNames, locale, onNavigate, text, t, today }: { readonly availabilityEvents: readonly EntityResult[]; readonly tasks: readonly EntityResult[]; readonly calendar?: EntityResult; readonly projectNames: ReadonlyMap<string, string>; readonly locale: Locale; readonly onNavigate: WorkspaceNavigate; readonly text: (document: Readonly<Record<string, unknown>>, key: string) => string; readonly t: (key: MessageKey, values?: Readonly<Record<string, string | number>>) => string; readonly today: string }) {
  const [month, setMonth] = useState(() => initialTaskMonth(tasks, text, today));
  ...
  return <section className="card people-task-calendar"><div className="people-calendar-heading"><div><h3>{t("people.schedule")}</h3><p>{t("people.scheduleDescription")}</p></div><div className="people-calendar-navigation"><button aria-label={t("people.previousMonth")} onClick={() => setMonth((current) => moveMonth(current, -1))} title={t("people.previousMonth")}>←</button><strong aria-live="polite">{monthLabel}</strong><button aria-label={t("people.nextMonth")} onClick={() => setMonth((current) => moveMonth(current, 1))} title={t("people.nextMonth")}>→</button></div></div>
    ...
    <div className="people-calendar-legend"><span className="today">{t("people.legendToday")}</span><span className="free">{t("people.legendFree")}</span><span className="busy">{t("people.legendBusy")}</span><span className="overlap">{t("people.legendOverlap")}</span><span className="unavailable">{t("availability.legendUnavailable")}</span><span className="off">{t("people.legendOff")}</span></div>
