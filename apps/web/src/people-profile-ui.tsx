...
function PersonEditorDrawer({ archivePerson, calendars, close, deletePerson, open, person, readOnly, restorePerson, savePerson, t }: { readonly archivePerson: () => Promise<boolean>; readonly calendars: readonly EntityResult[]; readonly close: () => void; readonly deletePerson: () => Promise<boolean>; readonly open: boolean; readonly person: EntityResult; readonly readOnly: boolean; readonly restorePerson: () => Promise<boolean>; readonly savePerson: (document: GitPmDocument) => Promise<boolean>; readonly t: (key: MessageKey, values?: Readonly<Record<string, string | number>>) => string }) {
  const name = text(person.document, "name");
  const [extraDays, setExtraDays] = useState(number(person.document, "annual_vacation_extra_days"));
  useEffect(() => { setExtraDays(number(person.document, "annual_vacation_extra_days")); }, [open, person.document]);
  const save = async (form: HTMLFormElement) => {
    const data = new FormData(form); const email = String(data.get("email") ?? ""); const calendar = String(data.get("calendar") ?? "");
    const nextExtraDays = Number(data.get("annual_vacation_extra_days"));
    const reason = String(data.get("annual_vacation_extra_days_reason") ?? "").trim();
    const { annual_vacation_extra_days: _extraDays, annual_vacation_extra_days_reason: _extraReason, ...unchanged } = person.document;
    return await savePerson({ ...unchanged, name: String(data.get("name") ?? ""), email: email || undefined, weekly_capacity_hours: Number(data.get("capacity")), calendar: calendar || undefined, ...(nextExtraDays > 0 ? { annual_vacation_extra_days: nextExtraDays, annual_vacation_extra_days_reason: reason } : {}) });
  };
  return <EditorDrawer closeLabel={t("core.closeEditor")} onClose={close} open={open} title={`${t("admin.editPerson")}: ${name}`}><form className="editor-drawer-form" key={person.draft_fingerprint} onSubmit={(event) => { event.preventDefault(); void save(event.currentTarget).then((success) => { if (success) close(); }); }}><label>{t("admin.personName")}<input name="name" defaultValue={name} required /></label><label>{t("admin.email")}<input name="email" defaultValue={text(person.document, "email")} type="email" /></label><label>{t("admin.capacity")}<span className="input-with-suffix"><input aria-label={t("admin.capacity")} name="capacity" type="number" min="0" step="0.25" defaultValue={number(person.document, "weekly_capacity_hours")} required /><span aria-hidden="true">{t("admin.hoursPerWeekUnit")}</span></span></label><label>{t("people.vacationExtraDays")}<input aria-label={t("people.vacationExtraDays")} min="0" name="annual_vacation_extra_days" onChange={(event) => setExtraDays(Number(event.target.value))} step="1" type="number" value={extraDays} /><small className="field-help">{t("people.vacationExtraHint")}</small></label><label>{t("people.vacationExtraReason")}<textarea aria-label={t("people.vacationExtraReason")} defaultValue={text(person.document, "annual_vacation_extra_days_reason")} name="annual_vacation_extra_days_reason" required={extraDays > 0} rows={3} /></label><label>{t("admin.calendar")}<select name="calendar" defaultValue={text(person.document, "calendar")}><option value="">{t("admin.defaultCalendar")}</option>{calendars.map((item) => <option key={item.document.id} value={item.document.id}>{text(item.document, "name")}</option>)}</select><small className="field-help">{t("admin.defaultCalendarHint")}</small></label><div className="editor-drawer-actions"><details className="more-actions"><summary>{t("core.moreActions")}</summary><div><button disabled={readOnly} onClick={() => { void archivePerson(); }} type="button">{t("core.archive")}</button><button className="danger" data-control-hint={t("controlHint.deleteEntity")} disabled={readOnly} onClick={() => { void deletePerson(); }} type="button">{t("core.delete")}</button></div></details><button onClick={close} type="button">{t("core.cancel")}</button><button className="primary" disabled={readOnly}>{t("core.save")}</button></div></form></EditorDrawer>;
}
