...
class BoardApi {
  ...
  entities: EntityResult[] = [
    ...
    this.result({ schema: "gitpm/task@1", id: taskId, project: projectId, title: "Drag me", type: "task", status: "backlog", lifecycle: "active" }),
  ];
  ...
  async listEntities(_draftId: string, type: string, project?: string) { const schemas: Record<string, string> = { projects: "gitpm/project@1", tasks: "gitpm/task@1", views: "gitpm/saved-view@1" }; return this.entities.filter((item) => item.document.schema === schemas[type] && (project === undefined || item.document.project === project)); }
...
describe("Board and Saved Views", () => {
  it("drags a Task between status columns and reopens persisted filters", async () => {
    ...
    expect(saved.document).toMatchObject({ kind: "board", group_by: "status", filters: { statuses: ["done"], types: ["task"] } });
