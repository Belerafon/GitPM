...
export function GanttWorkspace({ api, draft, locale, initialProjectId = "", onNavigate = () => undefined }: { readonly api: GitPmApi; readonly draft: DraftStatus; readonly locale: Locale; readonly initialProjectId?: string; readonly onNavigate?: WorkspaceNavigate }) {
  ...
  const primaryTrackHintId = `${markerPrefix}-primary-track-hint`;
  const additionalTracksHintId = `${markerPrefix}-additional-tracks-hint`;
  const dependencyTrackHintId = `${markerPrefix}-dependency-track-hint`;
  ...
  return <section className="gantt-workspace">
    ...
    <>
    <section className="card gantt-toolbar">{initialProjectId === "" && <label data-field-hint={t("fieldHint.ganttProject")}>{t("gantt.project")}<select value={projectId} onChange={(event) => onNavigate("gantt", { projectId: event.target.value })}>{projects.map((project) => <option key={project.document.id} value={project.document.id}>{stringValue(project.document, "name")}</option>)}</select></label>}<span>{t("gantt.visible", { count: model?.rows.length ?? 0 })}</span>{model !== null && <time className="gantt-range">{t("gantt.range", { start: formatDateOnly(locale, model.start), due: formatDateOnly(locale, model.due) })}</time>}<label className="gantt-scale">{t("gantt.scale")}<select value={dayWidth} onChange={(event) => setDayWidth(Number(event.target.value))}><option value="24">{t("gantt.scaleMonth")}</option><option value="36">{t("gantt.scaleWeek")}</option><option value="60">{t("gantt.scaleDay")}</option></select></label><span className="state open">{t("gantt.readOnly")}</span></section>
    {(dateTracks.length > 1 || dependencyTracks.length > 0) && <section className="card gantt-track-controls">
      <p className="gantt-track-names-hint">{t("gantt.trackNamesHint")} <button className="text-link" type="button" onClick={() => onNavigate("settings", { query: { section: ["planning"] } })}>{t("gantt.openScheduleTracks")}</button></p>
      <div className="gantt-track-fields">
        {dateTracks.length > 1 && <label data-field-hint={t("fieldHint.ganttPrimaryTrack")}>{t("gantt.primaryTrack")}<small className="gantt-track-field-hint" id={primaryTrackHintId}>{t("gantt.primaryTrackHint")}</small><select aria-describedby={primaryTrackHintId} aria-label={t("gantt.primaryTrack")} value={primaryTrack} onChange={(event) => setPrimaryTrack(event.target.value)}>{dateTracks.map((track) => <option key={track.slug} value={track.slug}>{track.title}</option>)}</select></label>}
        {dateTracks.length > 1 && <fieldset aria-describedby={additionalTracksHintId} className="gantt-additional-tracks"><legend data-field-hint={t("fieldHint.ganttAdditionalTracks")}>{t("gantt.additionalTracks")}</legend><small className="gantt-track-field-hint" id={additionalTracksHintId}>{t("gantt.additionalTracksHint")}</small>{dateTracks.filter((track) => track.slug !== primaryTrack).map((track) => <label key={track.slug}><input checked={selectedTracks.has(track.slug)} onChange={(event) => setSelectedTracks((current) => { const next = new Set(current); if (event.target.checked) next.add(track.slug); else next.delete(track.slug); return next; })} type="checkbox" />{track.title}</label>)}</fieldset>}
        {dependencyTracks.length > 0 && <label data-field-hint={t("fieldHint.ganttDependencyTrack")}>{t("gantt.dependencyTrack")}<small className="gantt-track-field-hint" id={dependencyTrackHintId}>{t("gantt.dependencyTrackHint")}</small><select aria-describedby={dependencyTrackHintId} aria-label={t("gantt.dependencyTrack")} value={dependencyTrack} onChange={(event) => setDependencyTrack(event.target.value)}>{dependencyTracks.map((track) => <option key={track.slug} value={track.slug}>{trackTitle(tracks, track.slug)}</option>)}</select></label>}
      </div>
    </section>}
