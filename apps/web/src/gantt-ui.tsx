  ...
  return <section className="gantt-workspace">
    ...
    {(dateTracks.length > 1 || dependencyTracks.length > 0) && <section className="card gantt-track-controls">
      ...
      <div className="gantt-track-fields">
        {dateTracks.length > 1 && <label data-field-hint={t("fieldHint.ganttPrimaryTrack")}>{t("gantt.primaryTrack")}<select aria-label={t("gantt.primaryTrack")} value={primaryTrack} onChange={(event) => setPrimaryTrack(event.target.value)}>{dateTracks.map((track) => <option key={track.slug} value={track.slug}>{track.title}</option>)}</select></label>}
        {dateTracks.length > 1 && <fieldset className="gantt-additional-tracks"><legend data-field-hint={t("fieldHint.ganttAdditionalTracks")}>{t("gantt.additionalTracks")}</legend>{dateTracks.filter((track) => track.slug !== primaryTrack).map((track) => <label key={track.slug}><input checked={selectedTracks.has(track.slug)} onChange={(event) => setSelectedTracks((current) => { const next = new Set(current); if (event.target.checked) next.add(track.slug); else next.delete(track.slug); return next; })} type="checkbox" />{track.title}</label>)}</fieldset>}
        {dependencyTracks.length > 0 && <label data-field-hint={t("fieldHint.ganttDependencyTrack")}>{t("gantt.dependencyTrack")}<select aria-label={t("gantt.dependencyTrack")} value={dependencyTrack} onChange={(event) => setDependencyTrack(event.target.value)}>{dependencyTracks.map((track) => <option key={track.slug} value={track.slug}>{trackTitle(tracks, track.slug)}</option>)}</select></label>}
      </div>
    </section>}
