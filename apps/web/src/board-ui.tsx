...
export function BoardWorkspace({ api, draft, locale, initialProjectId = "", initialStatusFilter = "", initialTypeFilter = "", initialMilestoneFilter = "", initialViewId = "", confirmAction = () => true, onNavigate = () => undefined, onChanged }: {
  ...
  return <section className="board-workspace">
    ...
    <section className="card board-toolbar">
      {initialProjectId === "" && <label data-field-hint={t("fieldHint.boardProject")}>{t("board.project")}<select aria-label={t("board.project")} value={projectId} onChange={(event) => onNavigate("board", { projectId: event.target.value })}>{projects.map((project) => <option key={project.document.id} value={project.document.id}>{text(project.document, "name")}</option>)}</select></label>}
    ...
    <div className="board-scroll-tools"><span>{t("board.scrollHint")}</span><div><button aria-label={t("board.previousColumns")} onClick={() => scrollColumns(-1)} title={t("board.previousColumns")} type="button">←</button><button aria-label={t("board.nextColumns")} onClick={() => scrollColumns(1)} title={t("board.nextColumns")} type="button">→</button></div></div>
