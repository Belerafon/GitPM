...
import { buildSchedule, scheduleEffort, scheduleText } from "./schedules.js";
...
const value = (document: GitPmDocument, key: string) => key === "start" || key === "due" ? scheduleText(document, key) : typeof document[key] === "string" ? document[key] as string : "";
const effortString = (document: GitPmDocument): string => { const effort = scheduleEffort(document); return typeof effort === "number" ? String(effort) : ""; };
...
function EntityEditor({ api, entity, entityType, draft, existingGroups = [], fingerprint, readOnly, externalFields, t, selected = false, detail = false, statusLabel, select, openTasks, openBoard, openGantt, milestoneTaskCount, milestoneCompletedCount, openMilestoneTasks, confirmArchive, confirmDelete, save, remove }: {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const document = { ...entity.document, name: String(data.get("name")), description_markdown: String(data.get("description")), ...(entityType === "milestones" ? { schedules: buildSchedule("", due, "") } : {}) } as EntityDocument;
...
function TaskReferenceSelect({ disabled, emptyLabel, label, onChange, selectedId, tasks }: { readonly disabled: boolean; readonly emptyLabel: string; readonly label: string; readonly onChange: ChangeEventHandler<HTMLSelectElement>; readonly selectedId: string; readonly tasks: readonly EntityResult[] }) {
  ...
  const selectedTitle = selectedId === "" ? emptyLabel : selectedTask === undefined ? "" : value(selectedTask.document, "title");
  return <label className="task-reference-field">{label}<span className="task-reference-control"><select aria-label={label} className="task-reference-select" disabled={disabled} onChange={onChange} value={selectedId}><option value="">{emptyLabel}</option>{tasks.map((item) => <option key={item.document.id} value={item.document.id}>{value(item.document, "title")}</option>)}</select>{selectedTitle !== "" && <span aria-hidden="true" className="task-reference-tooltip">{selectedTitle}</span>}</span></label>;
}

export function TaskPanel({ api, catalog, draft, entity, fingerprint, milestones, people, projects, tasks, readOnly, externalFields, locale, statusOptions, typeOptions, confirmDelete, confirmCommentDelete, focusedCommentId, onNavigate, onDeleted, onCommentChanged, onStatusChange, statusBusy = false, save, remove }: { readonly api: GitPmApi; readonly catalog: EntityCatalog; readonly draft: DraftStatus; readonly entity: EntityResult; readonly fingerprint: string; readonly milestones: readonly EntityResult[]; readonly people: readonly EntityResult[]; readonly projects: readonly EntityResult[]; readonly tasks: readonly EntityResult[]; readonly readOnly: boolean; readonly externalFields?: readonly string[]; readonly locale: Locale; readonly statusOptions: readonly ConfigValue[]; readonly typeOptions: readonly ConfigValue[]; readonly confirmDelete: (name: string) => boolean; readonly confirmCommentDelete: () => boolean; readonly focusedCommentId?: string; readonly onNavigate: WorkspaceNavigate; readonly onDeleted: () => void; readonly onCommentChanged: (fingerprint: string) => Promise<void>; readonly onStatusChange?: (status: string) => void; readonly statusBusy?: boolean; readonly save: (operation: () => Promise<EntityResult>) => Promise<EntityResult | null>; readonly remove: (operation: () => Promise<void>) => Promise<boolean> }) {
  const t = (key: MessageKey, values?: Readonly<Record<string, string | number>>) => message(locale, key, values);
  ...
  const descendantEstimate = descendants.reduce((sum, item) => sum + (scheduleEffort(item.entity.document) ?? 0), 0);
