...
import { activeProjectIds, ENTITY_ID_PREFIX, isOperationalTask, newUniqueEntityId, tokenizeProjectFileReferences } from "@gitpm/shared";
...
import { ProjectFileMarkdownField, renderProjectFileReferenceText, type ProjectFileReferenceContext } from "./project-file-reference-ui.js";
...
function boldMarkdown(text: string): ReactNode[] {
  return text.split(/(\*\*[^*]+\*\*)/gu).map((part, index) => part.startsWith("**") && part.endsWith("**")
    ? <strong key={index}>{part.slice(2, -2)}</strong>
    : <Fragment key={index}>{part}</Fragment>);
}

function inlineMarkdown(text: string, fileContext?: ProjectFileReferenceContext): ReactNode[] {
  if (fileContext === undefined) return boldMarkdown(text);
  ...
  const expand = (part: string, key: string): ReactNode[] => part.split(new RegExp(`(${marker}\\d+${marker})`, "gu")).filter(Boolean).map((piece, index) => {
    ...
    return match === null
      ? <Fragment key={`${key}:${index}`}>{piece}</Fragment>
...
export function SafeMarkdown({ fileContext, source }: { readonly fileContext?: ProjectFileReferenceContext; readonly source: string }) {
  return <div className="safe-markdown">{source.split(/\r?\n/u).map((line, index) => {
    if (line.startsWith("# ")) return <h4 key={index}>{inlineMarkdown(line.slice(2), fileContext)}</h4>;
    if (line.startsWith("- ")) return <div className="markdown-list-item" key={index}>• {inlineMarkdown(line.slice(2), fileContext)}</div>;
    return line === "" ? <br key={index} /> : <p key={index}>{inlineMarkdown(line, fileContext)}</p>;
  })}</div>;
}
