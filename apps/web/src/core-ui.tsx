...
export function CoreWorkspace({ api, draft, locale, surface = "projects", initialProjectId = "", initialTaskId = "", initialStatusFilter = "", initialMilestoneFilter = "", onNavigate = () => undefined, confirmAction = () => true, onChanged }: {
  ...
  return <section className={`core-workspace core-${surface}-workspace${reducedMotion ? " reduced-motion" : ""}`} data-reduced-motion={reducedMotion} data-surface={surface}>
    ...
    {surface === "tasks" && (task !== undefined ? <div className="task-detail-page"><button className="text-link back-link" onClick={() => onNavigate("tasks", { projectId, query: taskQuery() })}>← {t("core.backToTasks")}</button><TaskPanel api={api} catalog={catalog} confirmDelete={confirmDelete} draft={draft} entity={task} fingerprint={fingerprint} milestones={milestones} people={people} projects={activeProjects} readOnly={readOnly} externalFields={highlights[task.document.id]} locale={locale} statusOptions={statusOptions} typeOptions={typeOptions} onNavigate={onNavigate} onDeleted={() => onNavigate("tasks", { projectId })} onStatusChange={(status) => changeTaskStatus(task, status)} save={mutate} remove={remove} statusBusy={statusPending !== null} /></div> : selectedTask !== "" ? <div className="card empty-workspace"><p>{t("core.taskNotFound")}</p><button onClick={() => onNavigate("tasks", { projectId, query: taskQuery() })}>{t("core.backToTasks")}</button></div> : <section className="card task-area"><div className="task-toolbar"><div><h3>{projectId === "" ? t("core.allTasks") : t("core.tasksFor", { project: selectedProjectName })}</h3><p>{t(projectId === "" ? "core.allTasksHint" : "core.projectTasksHint")}</p></div><div className="task-toolbar-controls"><label>{t("core.project")}<select aria-label={t("core.project")} value={projectId} onChange={(event) => onNavigate("tasks", { projectId: event.target.value, query: taskQuery(filter, "") })}><option value="">{t("core.chooseProjectOption")}</option>{activeProjects.map((project) => <option key={project.document.id} value={project.document.id}>{value(project.document, "name")}</option>)}</select></label><label>{t("core.filter")}<select value={filter} onChange={(event) => onNavigate("tasks", { projectId, query: taskQuery(event.target.value, milestoneFilter) })}><option value="">{t("core.allStatuses")}</option>{statuses.map((status) => <option key={status} value={status}>{statusTitle(status)}</option>)}</select></label><label>{t("core.milestone")}<select aria-label={t("core.milestone")} value={milestoneFilter} onChange={(event) => onNavigate("tasks", { projectId, query: taskQuery(filter, event.target.value) })}><option value="">{t("core.allMilestones")}</option>{projectId !== "" && <option value="none">{t("stages.withoutStage")}</option>}{filterMilestones.map((milestone) => <option key={milestone.document.id} value={milestone.document.id}>{projectId === "" ? `${catalog.project(milestone.document.project).name} · ` : ""}{value(milestone.document, "name")}</option>)}</select></label></div></div>
      ...
      <div className="task-table">{filteredTasks.length === 0 ? <p>{t("core.empty")}</p> : filteredTasks.map((item) => { const assignees = values(item.document, "assignees").map(personName); return <div className={`task-row${highlights[item.document.id]?.includes("$local") ? " recently-changed" : highlights[item.document.id] ? " external-update" : ""}${statusPending === item.document.id ? " is-saving" : ""}`} data-external-fields={highlights[item.document.id]?.join(",")} key={item.document.id}><button onClick={() => onNavigate("tasks", { projectId: value(item.document, "project"), taskId: item.document.id, query: taskQuery() })}><strong>{value(item.document, "title")}</strong><code>{item.document.id}</code>{projectId === "" && <span>{catalog.project(item.document.project).name}</span>}{catalog.milestone(item.document.milestone) !== undefined && <span className="task-milestone">{catalog.milestone(item.document.milestone)?.name}</span>}<span className="task-assignees">{assignees.length === 0 ? t("core.unassigned") : assignees.join(", ")}</span></button>{readOnly ? <span className="state open">{statusTitle(value(item.document, "status"))}</span> : <select aria-label={`${t("core.status")}: ${value(item.document, "title")}`} className="inline-status-select" disabled={statusPending !== null} onChange={(event) => changeTaskStatus(item, event.target.value)} value={value(item.document, "status")}>{statusOptions.map((status) => <option key={status.slug} value={status.slug}>{status.title}</option>)}</select>}</div>; })}</div>
    </section>)}
    </>
    </AsyncBoundary>
  </section>;
...
export function TaskPanel({ api, catalog, draft, entity, fingerprint, milestones, people, projects, readOnly, externalFields, locale, statusOptions, typeOptions, confirmDelete, onNavigate, onDeleted, onStatusChange, statusBusy = false, save, remove }: { readonly api: GitPmApi; readonly catalog: EntityCatalog; readonly draft: DraftStatus; readonly entity: EntityResult; readonly fingerprint: string; readonly milestones: readonly EntityResult[]; readonly people: readonly EntityResult[]; readonly projects: readonly EntityResult[]; readonly readOnly: boolean; readonly externalFields?: readonly string[]; readonly locale: Locale; readonly statusOptions: readonly ConfigValue[]; readonly typeOptions: readonly ConfigValue[]; readonly confirmDelete: (name: string) => boolean; readonly onNavigate: WorkspaceNavigate; readonly onDeleted: () => void; readonly onStatusChange?: (status: string) => void; readonly statusBusy?: boolean; readonly save: (operation: () => Promise<EntityResult>) => Promise<EntityResult | null>; readonly remove: (operation: () => Promise<void>) => Promise<boolean> }) {
  const t = (key: MessageKey, values?: Readonly<Record<string, string | number>>) => message(locale, key, values);
  ...
  const assigneeNames = values(entity.document, "assignees").map((id) => value(people.find((person) => person.document.id === id)?.document ?? { schema: "", id: "", lifecycle: "active" }, "name") || id);
  ...
  return <section className={`card task-detail-card${externalFields?.includes("$local") ? " recently-changed" : externalFields ? " external-update" : ""}`} data-external-fields={externalFields?.join(",")}>
    ...
    <dl className="task-detail-meta"><div><dt>{t("core.project")}</dt><dd><button className="text-link" onClick={() => onNavigate("projects", { projectId: references.project.id })}>{references.project.name}</button></dd></div><div><dt>{t("core.type")}</dt><dd>{typeTitle}</dd></div><div><dt>{t("core.milestone")}</dt><dd>{references.milestone === undefined ? t("core.noMilestone") : <button className="text-link" onClick={() => onNavigate("stages", { projectId: references.project.id, stageId: references.milestoneId })}>{references.milestone.name}{references.milestone.lifecycle === "archived" && <small className="archived-reference"> · {t("core.archived")}</small>}</button>}</dd></div><div><dt>{t("core.assignees")}</dt><dd>{assigneeNames.length === 0 ? t("core.unassigned") : assigneeNames.join(", ")}</dd></div><div><dt>{t("projectPlan.start")}</dt><dd>{value(entity.document, "start") ? formatDateOnly(locale, value(entity.document, "start")) : "—"}</dd></div><div><dt>{t("core.due")}</dt><dd>{value(entity.document, "due") ? formatDateOnly(locale, value(entity.document, "due")) : "—"}</dd></div><div><dt>{t("projectPlan.estimate")}</dt><dd>{typeof entity.document.estimate_hours === "number" ? `${entity.document.estimate_hours} h` : "—"}</dd></div></dl>
