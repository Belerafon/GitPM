...
export function newEntityId(prefix: "PRJ" | "MLS" | "TSK", random: () => number = Math.random): string {
  let body = "";
  ...
  return <section className="core-workspace">
    ...
    <div className="core-columns">
      ...
      <section className="card entity-column"><h3>{t("core.milestones")}</h3>{projectId === "" ? <p>{t("core.selectProject")}</p> : <>
        ...
        <div className="entity-list">{activeMilestones.map((milestone) => <EntityEditor api={api} key={milestone.document.id} entity={milestone} entityType="milestones" draft={draft} fingerprint={fingerprint} readOnly={readOnly} t={t} save={mutate} remove={remove} />)}</div>
      </>}</section>
    </div>
    {projectId !== "" && <section className="card task-area"><div className="task-toolbar"><h3>{t("core.tasks")}</h3><label>{t("core.filter")}<select value={filter} onChange={(event) => setFilter(event.target.value)}><option value="">{t("core.allStatuses")}</option>{statuses.map((status) => <option key={status} value={status}>{status}</option>)}</select></label></div>
      ...
      <div className="task-layout"><div className="task-table">{filteredTasks.length === 0 ? <p>{t("core.empty")}</p> : filteredTasks.map((item) => <div className="task-row" key={item.document.id}><button onClick={() => setSelectedTask(item.document.id)}><strong>{value(item.document, "title")}</strong><code>{item.document.id}</code></button><select aria-label={`${t("core.status")} ${value(item.document, "title")}`} disabled={readOnly} value={value(item.document, "status")} onChange={(event) => { void mutate(async () => await api.updateEntity(draft.draft_id, "tasks", item, fingerprint, { ...item.document, status: event.target.value })); }}>{statuses.map((status) => <option key={status}>{status}</option>)}</select><button disabled={readOnly} onClick={() => { void mutate(async () => await api.archiveEntity(draft.draft_id, "tasks", item, fingerprint)); if (selectedTask === item.document.id) setSelectedTask(""); }}>{t("core.archive")}</button></div>)}</div>
        {task !== undefined && <TaskPanel api={api} draft={draft} entity={task} fingerprint={fingerprint} milestones={activeMilestones} readOnly={readOnly} locale={locale} save={mutate} remove={remove} />}
      </div>
    </section>}
  </section>;
}

function EntityEditor({ api, entity, entityType, draft, fingerprint, readOnly, t, selected = false, select, save, remove }: {
  readonly api: GitPmApi; readonly entity: EntityResult; readonly entityType: "projects" | "milestones"; readonly draft: DraftStatus; readonly fingerprint: string; readonly readOnly: boolean; readonly t: (key: MessageKey) => string; readonly selected?: boolean; readonly select?: () => void;
  ...
  return <form className={selected ? "entity-card selected" : "entity-card"} onSubmit={submit}>
    {select !== undefined && <button type="button" className="entity-select" onClick={select}><code>{entity.document.id}</code></button>}
    <input name="name" aria-label={`${t("core.name")} ${value(entity.document, "name")}`} defaultValue={value(entity.document, "name")} required />
    {entityType === "milestones" && <input name="due" type="date" aria-label={`${t("core.due")} ${value(entity.document, "name")}`} defaultValue={value(entity.document, "due")} />}
    <textarea name="description" aria-label={`${t("core.description")} ${value(entity.document, "name")}`} defaultValue={value(entity.document, "description_markdown")} />
...
function TaskPanel({ api, draft, entity, fingerprint, milestones, readOnly, locale, save, remove }: { readonly api: GitPmApi; readonly draft: DraftStatus; readonly entity: EntityResult; readonly fingerprint: string; readonly milestones: readonly EntityResult[]; readonly readOnly: boolean; readonly locale: Locale; readonly save: (operation: () => Promise<EntityResult>) => Promise<void>; readonly remove: (operation: () => Promise<void>) => Promise<void> }) {
