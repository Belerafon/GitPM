...
import { buildSchedule, scheduleEffort, scheduleText, ScheduleResolver, scheduleTracksConfig, withScheduleWindow } from "./schedules.js";
...
export function CoreWorkspace({ api, draft, locale, surface = "projects", initialProjectId = "", initialTaskId = "", initialCommentId = "", initialStatusFilter = "", initialMilestoneFilter = "", onNavigate = () => undefined, confirmAction = () => true, onChanged }: {
  ...
  const createMilestone = (event: FormEvent<HTMLFormElement>) => {
    ...
    const document = { schema: "gitpm/milestone@2", id: newUniqueEntityId(ENTITY_ID_PREFIX.milestone, new Set(milestones.map((item) => item.document.id))), project: projectId, name: String(data.get("name")), lifecycle: "active", description_markdown: String(data.get("description")), ...(buildSchedule(projectTrack(projectId), "", String(data.get("due") ?? ""), "") ? { schedules: buildSchedule(projectTrack(projectId), "", String(data.get("due") ?? ""), "") } : {}) } as EntityDocument;
  ...
  return <section className={`core-workspace core-${surface}-workspace${reducedMotion ? " reduced-motion" : ""}`} data-reduced-motion={reducedMotion} data-surface={surface}>
    ...
    {surface === "projects" && (projectId === "" ? <section className="project-directory"><div className="card-heading"><div><h3>{t("core.projectList")}</h3><p>{t("core.portfolioDescription")}</p></div><button className="primary" disabled={readOnly} onClick={() => { setError(null); setCreateEditor("project"); }} type="button">+ {t("core.createProjectAction")}</button><EditorDrawer closeLabel={t("core.closeEditor")} onClose={() => setCreateEditor(null)} open={createEditor === "project"} title={t("core.createProjectAction")}><form className="editor-drawer-form" onSubmit={createProject}><label>{t("core.name")}<input disabled={readOnly} name="name" required /></label><ProjectGroupField currentGroup="" disabled={readOnly} groups={existingGroups} key={createEditor === "project" ? "open" : "closed"} t={t} /><label>{t("core.description")}<textarea disabled={readOnly} name="description" /></label><div className="editor-drawer-actions"><button onClick={() => setCreateEditor(null)} type="button">{t("core.cancel")}</button><button className="primary" disabled={readOnly}>{t("core.createProject")}</button></div></form></EditorDrawer></div>
      ...
      <EntityEditor api={api} confirmDelete={confirmDelete} detail entity={selectedProject} entityType="projects" value={value} track={projectTrack(selectedProject.document.id)} draft={draft} existingGroups={existingGroups} fingerprint={fingerprint} readOnly={readOnly} externalFields={highlights[selectedProject.document.id]} t={t} statusLabel={statusTitle(value(selectedProject.document, "status"))} openTasks={() => onNavigate("tasks", { projectId })} openBoard={() => onNavigate("board", { projectId })} openGantt={() => onNavigate("gantt", { projectId })} save={mutate} remove={remove} />
...
function EntityEditor({ api, entity, entityType, draft, existingGroups = [], fingerprint, readOnly, externalFields, t, selected = false, detail = false, statusLabel, select, openTasks, openBoard, openGantt, milestoneTaskCount, milestoneCompletedCount, openMilestoneTasks, confirmArchive, confirmDelete, value, track, save, remove }: {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const base = { ...entity.document, name: String(data.get("name")), description_markdown: String(data.get("description")) };
    const document = (entityType === "milestones" ? withScheduleWindow(base, track, { finish: due }) : base) as EntityDocument;
...
function TaskReferenceSelect({ disabled, emptyLabel, label, onChange, selectedId, tasks }: { readonly disabled: boolean; readonly emptyLabel: string; readonly label: string; readonly onChange: ChangeEventHandler<HTMLSelectElement>; readonly selectedId: string; readonly tasks: readonly EntityResult[] }) {
  ...
  return <label className="task-reference-field">{label}<span className="task-reference-control"><select aria-label={label} className="task-reference-select" disabled={disabled} onChange={onChange} value={selectedId}><option value="">{emptyLabel}</option>{tasks.map((item) => <option key={item.document.id} value={item.document.id}>{stringValue(item.document, "title")}</option>)}</select>{selectedTitle !== "" && <span aria-hidden="true" className="task-reference-tooltip">{selectedTitle}</span>}</span></label>;
}

export function TaskPanel({ api, catalog, draft, entity, fingerprint, milestones, people, projects, tasks, readOnly, externalFields, locale, statusOptions, typeOptions, confirmDelete, confirmCommentDelete, focusedCommentId, onNavigate, onDeleted, onCommentChanged, onStatusChange, statusBusy = false, value, effortString, track, save, remove }: { readonly api: GitPmApi; readonly catalog: EntityCatalog; readonly draft: DraftStatus; readonly entity: EntityResult; readonly fingerprint: string; readonly milestones: readonly EntityResult[]; readonly people: readonly EntityResult[]; readonly projects: readonly EntityResult[]; readonly tasks: readonly EntityResult[]; readonly readOnly: boolean; readonly externalFields?: readonly string[]; readonly locale: Locale; readonly statusOptions: readonly ConfigValue[]; readonly typeOptions: readonly ConfigValue[]; readonly confirmDelete: (name: string) => boolean; readonly confirmCommentDelete: () => boolean; readonly focusedCommentId?: string; readonly onNavigate: WorkspaceNavigate; readonly onDeleted: () => void; readonly onCommentChanged: (fingerprint: string) => Promise<void>; readonly onStatusChange?: (status: string) => void; readonly statusBusy?: boolean; readonly value: ScheduleTextReader; readonly effortString: (document: Readonly<Record<string, unknown>>) => string; readonly track: string; readonly save: (operation: () => Promise<EntityResult>) => Promise<EntityResult | null>; readonly remove: (operation: () => Promise<void>) => Promise<boolean> }) {
  const t = (key: MessageKey, values?: Readonly<Record<string, string | number>>) => message(locale, key, values);
  ...
  const [estimate, setEstimate] = useState(effortString(entity.document));
  ...
  useEffect(() => { setTitle(value(entity.document, "title")); setStatus(value(entity.document, "status")); setType(value(entity.document, "type")); setDescription(value(entity.document, "description_markdown")); setMilestone(value(entity.document, "milestone")); setParent(value(entity.document, "parent")); setAssignees(values(entity.document, "assignees")); setStart(value(entity.document, "start")); setDue(value(entity.document, "due")); setEstimate(effortString(entity.document)); }, [entity]);
  ...
  return <section className={`card task-detail-card${externalFields?.includes("$local") ? " recently-changed" : externalFields ? " external-update" : ""}`} data-external-fields={externalFields?.join(",")}>
    ...
    <div className="editor-actions"><button className="primary" disabled={readOnly} onClick={() => setEditor("subtask")} type="button">+ {t("taskHierarchy.newSubtask")}</button><button className="editor-trigger" onClick={() => { setTitle(value(entity.document, "title")); setStatus(entityStatus); setType(entityType); setDescription(value(entity.document, "description_markdown")); setMilestone(value(entity.document, "milestone")); setParent(value(entity.document, "parent")); setAssignees(values(entity.document, "assignees")); setStart(value(entity.document, "start")); setDue(value(entity.document, "due")); setEstimate(effortString(entity.document)); setEditor("edit"); }} type="button">{t("core.edit")}</button><button onClick={() => { setTargetProject(""); setTargetMilestone(""); setTargetParent(""); setEditor("move"); }} type="button">{t("core.moveTask")}</button></div>
    <TaskTimeEntries api={api} draft={draft} fingerprint={fingerprint} locale={locale} onFingerprintChange={onCommentChanged} people={people} projectId={references.project.id} readOnly={readOnly} taskId={entity.document.id} assigneeIds={assigneeIds} />
