...
import {
  countViewConditions, emptyViewQuery, filterOnlyViewQuery, newViewNodeId, operatorsFor, removeViewFilterNode,
...
export function AdvancedViewControls<Row>({ fields, locale, query, onChange, resultCount, totalCount, leadingControls, appliedControls, externalFilterCount = 0, groupLabel, onClear, quickPresets, allowSorting = true, t }: {
  ...
  const viewQuery = allowSorting ? query : filterOnlyViewQuery(query);
  ...
  const openEditor = () => { setDraft(viewQuery); setOpen(true); };
        ...
        <details className="advanced-view-custom" open>
          ...
          <div>
            {allowSorting && <section><header><h3>{t("advancedView.sorting")}</h3><button onClick={() => { const field = fields.find((candidate) => candidate.sortable !== false); if (field !== undefined) setDraft((current) => ({ ...current, sort: [...current.sort, { id: newViewNodeId("sort"), field: field.id, direction: "asc" }] })); }} type="button">+ {t("advancedView.addSort")}</button></header>
              <div className="advanced-sort-rules">{draft.sort.map((rule, index) => <div className="advanced-sort-rule" key={rule.id}><span>{index + 1}</span><select aria-label={t("advancedView.sortField", { number: index + 1 })} data-field-hint={t("fieldHint.sortField")} onChange={(event) => setDraft((current) => ({ ...current, sort: current.sort.map((candidate) => candidate.id === rule.id ? { ...candidate, field: event.target.value } : candidate) }))} value={rule.field}>{fields.filter((field) => field.sortable !== false).map((field) => <option key={field.id} value={field.id}>{field.label}</option>)}</select><select aria-label={t("advancedView.direction", { number: index + 1 })} data-field-hint={t("fieldHint.sortDirection")} onChange={(event) => setDraft((current) => ({ ...current, sort: current.sort.map((candidate) => candidate.id === rule.id ? { ...candidate, direction: event.target.value as "asc" | "desc" } : candidate) }))} value={rule.direction}><option value="asc">{t("advancedView.ascending")}</option><option value="desc">{t("advancedView.descending")}</option></select><button aria-label={t("advancedView.removeSort", { field: fieldMap.get(rule.field)?.label ?? rule.field })} onClick={() => setDraft((current) => ({ ...current, sort: current.sort.filter((candidate) => candidate.id !== rule.id) }))} type="button">×</button></div>)}</div>
            </section>}
            <section><header><h3>{t("advancedView.conditions")}</h3></header><FilterGroupEditor fields={fields} group={draft.filter} locale={locale} root onChange={(filter) => setDraft((current) => ({ ...current, filter }))} t={t} /></section>
          </div>
        </details>
...
function FilterConditionEditor<Row>({ condition, fields, locale, onChange, onRemove, t }: { readonly condition: ViewFilterCondition; readonly fields: readonly ViewField<Row>[]; readonly locale: Locale; readonly onChange: (condition: ViewFilterCondition) => void; readonly onRemove: () => void; readonly t: Translator }) {
  ...
  const hintId = field.hint === undefined ? undefined : `${condition.id}-field-hint`;
  return <div className="advanced-filter-condition"><div className="advanced-filter-condition-controls"><select aria-describedby={hintId} aria-label={t("advancedView.field")} onChange={(event) => setField(event.target.value)} value={field.id}>{fields.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.label}</option>)}</select><select aria-label={t("advancedView.operator")} onChange={(event) => onChange({ ...condition, operator: event.target.value as ViewFilterOperator })} value={condition.operator}>{operators.map((operator) => <option key={operator} value={operator}>{operatorLabel(operator, t)}</option>)}</select>{!noValue && valueInput("value")}{condition.operator === "between" && valueInput("valueTo")}<button aria-label={t("advancedView.removeCondition")} onClick={onRemove} type="button">×</button></div>{field.hint !== undefined && <p className="field-help advanced-filter-field-hint" id={hintId}>{field.hint}</p>}</div>;
}
...
function quickViewPresets<Row>(fields: readonly ViewField<Row>[], t: Translator, allowSorting: boolean): readonly QuickViewPreset[] {
