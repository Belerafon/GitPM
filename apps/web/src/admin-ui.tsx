...
export function AdminWorkspace({ api, draft, role, locale, surface, confirmAction = () => true, initialCalendarId, initialSection, onOpenCalendar, onOpenPerson, onOpenProject, onOpenView, onChanged }: { readonly api: GitPmApi; readonly draft: DraftStatus; readonly role: GitPmRole; readonly locale: Locale; readonly surface: AdminSurface; readonly confirmAction?: (message: string) => boolean; readonly initialCalendarId?: string; readonly initialSection?: string; readonly onOpenCalendar?: (calendarId: string) => void; readonly onOpenPerson?: (personId: string) => void; readonly onOpenProject?: (projectId: string) => void; readonly onOpenView?: (projectId: string, viewId: string) => void; readonly onChanged: () => Promise<void> }) {
  ...
  const personOptions = useMemo(() => people.map((person) => ({ value: person.document.id, label: text(person.document, "name") })), [people]);
  ...
  ], [calendarOptions, lifecycleOptions, locale, projectsByPerson, teams, teamsByPerson]);
  ...
  return <section className="admin-workspace"><div className="section-heading"><span className="eyebrow draft-context-id">{draft.draft_id}</span><h2 aria-hidden="true">{t(surfaceHeading)}</h2></div>
    ...
    {surface === "settings" && <section aria-label={t("admin.settings")} className="settings-config-sections">
      <section aria-labelledby="settings-tasks-heading" className="settings-config-section"><h2 id="settings-tasks-heading">{t("admin.settingsTasks")}</h2><div className="settings-config-grid">{statuses !== null && <ConfigEditor api={api} draft={draft} entity={statuses} kind="statuses" listKey="statuses" title={t("admin.statuses")} onOpenView={onOpenView} readOnly={readOnly} t={t} mutate={mutate} />}{issueTypes !== null && <ConfigEditor api={api} draft={draft} entity={issueTypes} kind="issue-types" listKey="issue_types" title={t("admin.issueTypes")} onOpenView={onOpenView} readOnly={readOnly} t={t} mutate={mutate} />}</div></section>
      <section aria-labelledby="settings-planning-heading" className="settings-config-section"><h2 id="settings-planning-heading">{t("admin.settingsPlanning")}</h2><div className="settings-config-grid">{scheduleTracks !== null && <ScheduleTracksConfigEditor api={api} draft={draft} entity={scheduleTracks} locale={locale} readOnly={readOnly} t={t} mutate={mutate} />}</div></section>
      <section aria-labelledby="settings-time-heading" className="settings-config-section"><h2 id="settings-time-heading">{t("admin.settingsTimeTracking")}</h2><div className="settings-config-grid">{workCategories !== null && <ConfigEditor api={api} draft={draft} entity={workCategories} kind="work-categories" listKey="categories" title={t("admin.workCategories")} showColor={false} onOpenView={onOpenView} readOnly={readOnly} t={t} mutate={mutate} />}</div></section>
    </section>}
    </>
    </AsyncBoundary>
  </section>;
