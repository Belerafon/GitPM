...
export function AdminWorkspace({ api, draft, role, locale, surface, confirmAction = () => true, initialCalendarId, onOpenCalendar, onOpenPerson, onOpenProject, onOpenView, onChanged }: { readonly api: GitPmApi; readonly draft: DraftStatus; readonly role: GitPmRole; readonly locale: Locale; readonly surface: AdminSurface; readonly confirmAction?: (message: string) => boolean; readonly initialCalendarId?: string; readonly onOpenCalendar?: (calendarId: string) => void; readonly onOpenPerson?: (personId: string) => void; readonly onOpenProject?: (projectId: string) => void; readonly onOpenView?: (projectId: string, viewId: string) => void; readonly onChanged: () => Promise<void> }) {
  ...
  return <section className="admin-workspace"><div className="section-heading"><span className="eyebrow draft-context-id">{draft.draft_id}</span><h2 aria-hidden="true">{t(surfaceHeading)}</h2></div>
    ...
    {surface === "settings" && <section aria-label={t("admin.settings")} className="settings-config-grid">{repository !== null && <RepositoryConfigEditor api={api} calendars={activeCalendars} draft={draft} entity={repository} locale={locale} onOpenCalendar={onOpenCalendar} readOnly={readOnly} t={t} mutate={mutate} />}{statuses !== null && <ConfigEditor api={api} draft={draft} entity={statuses} kind="statuses" listKey="statuses" title={t("admin.statuses")} onOpenView={onOpenView} readOnly={readOnly} t={t} mutate={mutate} />}{issueTypes !== null && <ConfigEditor api={api} draft={draft} entity={issueTypes} kind="issue-types" listKey="issue_types" title={t("admin.issueTypes")} onOpenView={onOpenView} readOnly={readOnly} t={t} mutate={mutate} />}{workCategories !== null && <ConfigEditor api={api} draft={draft} entity={workCategories} kind="work-categories" listKey="categories" title={t("admin.workCategories")} showColor={false} onOpenView={onOpenView} readOnly={readOnly} t={t} mutate={mutate} />}{scheduleTracks !== null && <ScheduleTracksConfigEditor api={api} draft={draft} entity={scheduleTracks} locale={locale} readOnly={readOnly} t={t} mutate={mutate} />}</section>}
    </>
    </AsyncBoundary>
  </section>;
}
...
function RepositoryConfigEditor({ api, calendars, draft, entity, locale, onOpenCalendar, readOnly, t, mutate }: { readonly api: GitPmApi; readonly calendars: readonly EntityResult[]; readonly draft: DraftStatus; readonly entity: RepositoryResult; readonly locale: Locale; readonly onOpenCalendar?: (calendarId: string) => void; readonly readOnly: boolean; readonly t: (key: MessageKey, values?: Readonly<Record<string, string | number>>) => string; readonly mutate: (operation: () => Promise<RepositoryResult>) => Promise<RepositoryResult | null> }) {
  ...
  return <article className="config-editor config-summary repository-config-editor">
    ...
    <section className="repository-default-calendar"><span>{t("admin.defaultCalendar")}</span>{calendar === undefined ? <code>{entity.document.default_calendar}</code> : <><button className="text-link repository-calendar-link" disabled={onOpenCalendar === undefined} onClick={() => openCalendar(calendar.document.id)} type="button">{String(calendar.document.name ?? calendar.document.id)}</button><code>{calendar.document.id}</code><CalendarPreview entity={calendar} locale={locale} t={t} /><p className="config-hint">{t("admin.defaultCalendarHint")}</p><button className="repository-calendar-open" disabled={onOpenCalendar === undefined} onClick={() => openCalendar(calendar.document.id)} type="button">{t("admin.openCalendar")}</button></>}
    </section>
    <dl className="repository-config-values"><div><dt>{t("admin.pollInterval")}</dt><dd>{t("admin.pollIntervalSeconds", { count: entity.document.ui_poll_interval_seconds })}</dd></div></dl>
    <EditorDrawer closeLabel={t("core.closeEditor")} onClose={close} open={open} title={`${t("core.edit")}: ${t("admin.repositoryConfiguration")}`}><form className="editor-drawer-form" onSubmit={submit}>
      ...
      {selectedCalendar !== undefined && <div className="repository-calendar-selection-preview"><code>{selectedCalendar.document.id}</code><CalendarPreview entity={selectedCalendar} locale={locale} t={t} /><p className="config-hint">{t("admin.defaultCalendarHint")}</p><button disabled={onOpenCalendar === undefined} onClick={() => openCalendar(selectedCalendar.document.id)} type="button">{t("admin.openSelectedCalendar")}</button></div>}
