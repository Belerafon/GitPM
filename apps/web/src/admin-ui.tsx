...
export function AdminWorkspace({ api, draft, role, locale, surface, confirmAction = () => true, initialCalendarId, initialSection, now, query, onNavigate, onOpenCalendar, onOpenPerson, onOpenProject, onOpenWorkload, onOpenView, onPersonNameFormatChanged, onChanged }: { readonly api: GitPmApi; readonly draft: DraftStatus; readonly role: GitPmRole; readonly locale: Locale; readonly surface: AdminSurface; readonly confirmAction?: (message: string) => boolean; readonly initialCalendarId?: string; readonly initialSection?: string; readonly now?: string; readonly query?: Readonly<Record<string, readonly string[]>>; readonly onNavigate?: WorkspaceNavigate; readonly onOpenCalendar?: (calendarId: string) => void; readonly onOpenPerson?: (personId: string) => void; readonly onOpenProject?: (projectId: string) => void; readonly onOpenWorkload?: (selection: { readonly personId?: string; readonly teamId?: string }) => void; readonly onOpenView?: (projectId: string, viewId: string) => void; readonly onPersonNameFormatChanged?: (format: PersonNameFormat) => void; readonly onChanged: () => Promise<void> }) {
  ...
  const settingsSection = initialSection === "planning" || initialSection === "time" || initialSection === "people" ? initialSection : "tasks";
  ...
  return <section className="admin-workspace"><div className="section-heading"><span className="eyebrow draft-context-id">{draft.draft_id}</span><h2 aria-hidden="true">{t(surfaceHeading)}</h2></div>
    ...
    {surface === "people" && <div className="people-admin-sections">
      <div className="catalog-tabs" role="tablist" aria-label={t("nav.people")}>
        <button aria-selected={peopleCatalog === "employees"} onClick={() => setPeopleCatalog("employees")} role="tab" type="button">{t("people.catalogEmployees", { count: people.length })}</button>
        <button aria-selected={peopleCatalog === "teams"} onClick={() => setPeopleCatalog("teams")} role="tab" type="button">{t("people.catalogTeams", { count: teams.length })}</button>
      </div>
      {peopleCatalog === "employees" && <section className="card directory-card people-directory-card">
        ...
        <EditorDrawer closeLabel={t("core.closeEditor")} onClose={() => setCreateEditor(null)} open={createEditor === "person"} title={t("admin.createPerson")}>
          <form className="editor-drawer-form" key={createEditor === "person" ? "person-open" : "person-closed"} onSubmit={createPerson}>
            ...
            <label data-field-hint={t("fieldHint.vacationExtraReason")}>{t("people.vacationExtraReason")}<textarea aria-label={t("people.vacationExtraReason")} name="annual_vacation_extra_days_reason" required={createPersonExtraDays > 0} rows={3} /></label>
    ...
    {surface === "settings" && <section aria-label={t(surfaceHeading)} className="settings-config-sections">
      ...
      {settingsSection === "people" && <section className="settings-config-section"><p className="admin-section-description">{t("admin.settingsPeopleDescription")}</p><div className="settings-config-grid">{repository !== null && <DefaultPersonNameFormatEditor api={api} draft={draft} entity={repository} readOnly={readOnly} t={t} mutate={mutate} onChanged={onPersonNameFormatChanged} />}</div></section>}
    </section>}
    </>
    </AsyncBoundary>
  </section>;
    ...
    <EditorDrawer closeLabel={t("core.closeEditor")} onClose={close} open={open} size="wide" title={`${t("core.edit")}: ${title}`}><form className="editor-drawer-form config-editor-form" onSubmit={submit} ref={formRef}>
      ...
      <section className="schedule-tracks-step">
        ...
        <div className="schedule-track-editor-fields">
          ...
          <fieldset className="config-field schedule-track-kind-options"><legend data-field-hint={t("admin.scheduleTracksDefinitionsHint")}>{t("admin.trackDataEntry")}</legend><div>
          ...
          {track.kind === "manual" ? <fieldset className="config-field schedule-track-capabilities"><legend data-field-hint={t("admin.trackDataFieldsHint")}>{t("admin.trackDataFields")}</legend><p>{t("admin.trackDataFieldsHint")}</p><div>{(["dates", "effort", "dependencies"] as const).map((capability) => <label key={capability}><input aria-label={capabilityTitle(capability)} checked={track.capabilities?.includes(capability) ?? false} disabled={readOnly || busy || (track.capabilities?.length === 1 && track.capabilities[0] === capability)} onChange={() => toggleCapability(index, capability)} type="checkbox" /><span><strong>{capabilityTitle(capability)}</strong><small>{capabilityDescription(capability)}</small></span></label>)}</div></fieldset> : <section className="schedule-track-actual-source"><strong>{t("admin.actualTrackCalculationTitle")}</strong><p>{t("admin.actualTrackCalculationHint")}</p></section>}
