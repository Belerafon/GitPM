...
import type { ConfigurationDocument, ConfigurationResult, DraftStatus, EntityResult, GitPmDocument, GitPmRole, ProjectPlanning, RepositoryResult } from "./types.js";
...
function ConfigEditor({ api, draft, entity, kind, listKey, title, showColor = true, readOnly, t, mutate }: { readonly api: GitPmApi; readonly draft: DraftStatus; readonly entity: ConfigurationResult; readonly kind: "statuses" | "issue-types" | "work-categories"; readonly listKey: "statuses" | "issue_types" | "categories"; readonly title: string; readonly showColor?: boolean; readonly readOnly: boolean; readonly t: (key: MessageKey, values?: Readonly<Record<string, string | number>>) => string; readonly mutate: <Result extends EntityResult | ConfigurationResult>(operation: () => Promise<Result>) => Promise<Result | null> }) {
  ...
  const close = () => { setValues(entityValues); setNewSlug(""); setOpen(false); };
...
function ScheduleTracksConfigEditor({ api, draft, entity, locale, readOnly, t, mutate }: { readonly api: GitPmApi; readonly draft: DraftStatus; readonly entity: ConfigurationResult; readonly locale: Locale; readonly readOnly: boolean; readonly t: (key: MessageKey, values?: Readonly<Record<string, string | number>>) => string; readonly mutate: <Result extends EntityResult | ConfigurationResult>(operation: () => Promise<Result>) => Promise<Result | null> }) {
  ...
  const reset = () => { setTracks(entityTracks); setDefaults(resolvedDefaults); setDefaultsTouched(false); setNewTrackSlug(""); };
