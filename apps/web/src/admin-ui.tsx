...
export function AdminWorkspace({ api, draft, role, locale, surface, confirmAction = () => true, onChanged }: { readonly api: GitPmApi; readonly draft: DraftStatus; readonly role: GitPmRole; readonly locale: Locale; readonly surface: AdminSurface; readonly confirmAction?: (message: string) => boolean; readonly onChanged: () => Promise<void> }) {
  ...
  const mutate = async (operation: () => Promise<EntityResult>) => { setError(null); try {
    ...
    await onChanged(); await load(); return result;
  ...
  return <section className="admin-workspace"><div className="section-heading"><span className="eyebrow draft-context-id">{draft.draft_id}</span><h2 aria-hidden="true">{t(surfaceHeading)}</h2></div>
    ...
    <>
    {surface === "calendar" && <section className="card"><button className="primary editor-trigger" disabled={readOnly} onClick={() => setCreateEditor("calendar")} type="button">+ {t("admin.createCalendar")}</button><EditorDrawer closeLabel={t("core.closeEditor")} onClose={() => setCreateEditor(null)} open={createEditor === "calendar"} title={t("admin.createCalendar")}><form className="editor-drawer-form" onSubmit={createCalendar}><label>{t("core.name")}<input name="name" required /></label><WeekdayChecks selected={[1, 2, 3, 4, 5]} t={t} /><label>{t("admin.holidays")}<input name="holidays" placeholder="2026-01-01" /></label><div className="editor-drawer-actions"><button onClick={() => setCreateEditor(null)} type="button">{t("core.cancel")}</button><button className="primary" disabled={readOnly}>{t("admin.createCalendar")}</button></div></form></EditorDrawer><div className="admin-grid">{activeCalendars.map((entity) => <div className={highlights[entity.document.id] ? "recently-changed" : ""} key={entity.document.id}><CalendarEditor {...{ api, draft, entity, fingerprint, readOnly, t, locale, mutate, remove, confirmDelete }} /></div>)}</div></section>}
    {surface === "people" && <div className="admin-columns"><section className="card"><div className="card-heading"><h3>{t("admin.people")}</h3><label className="search-field">{t("admin.peopleSearch")}<input type="search" value={peopleQuery} onChange={(event) => setPeopleQuery(event.target.value)} /></label></div><button className="primary editor-trigger" disabled={readOnly || activeCalendars.length === 0} onClick={() => setCreateEditor("person")} type="button">+ {t("admin.createPerson")}</button><EditorDrawer closeLabel={t("core.closeEditor")} onClose={() => setCreateEditor(null)} open={createEditor === "person"} title={t("admin.createPerson")}><form className="editor-drawer-form" onSubmit={createPerson}><label>{t("core.name")}<input name="name" required /></label><label>{t("admin.email")}<input name="email" type="email" /></label><label>{t("admin.capacity")}<input name="capacity" type="number" min="0" step="0.25" defaultValue="40" required /></label><label>{t("admin.calendar")}<select name="calendar" required>{activeCalendars.map((item) => <option value={item.document.id} key={item.document.id}>{text(item.document, "name")}</option>)}</select></label><div className="editor-drawer-actions"><button onClick={() => setCreateEditor(null)} type="button">{t("core.cancel")}</button><button className="primary" disabled={readOnly || activeCalendars.length === 0}>{t("admin.createPerson")}</button></div></form></EditorDrawer><div className="admin-grid">{visiblePeople.map((entity) => <div className={highlights[entity.document.id] ? "recently-changed" : ""} key={entity.document.id}><PersonEditor {...{ api, draft, entity, fingerprint, readOnly, t, calendars: activeCalendars, mutate, remove, confirmDelete }} /></div>)}</div></section>
      <section className="card"><div className="card-heading"><h3>{t("admin.teams")}</h3><label className="search-field">{t("admin.teamSearch")}<input type="search" value={teamQuery} onChange={(event) => setTeamQuery(event.target.value)} /></label></div><button className="primary editor-trigger" disabled={readOnly} onClick={() => setCreateEditor("team")} type="button">+ {t("admin.createTeam")}</button><EditorDrawer closeLabel={t("core.closeEditor")} onClose={() => setCreateEditor(null)} open={createEditor === "team"} title={t("admin.createTeam")}><form className="editor-drawer-form" onSubmit={createTeam}><label>{t("core.name")}<input name="name" required /></label><MemberChecks people={activePeople} selected={[]} t={t} /><div className="editor-drawer-actions"><button onClick={() => setCreateEditor(null)} type="button">{t("core.cancel")}</button><button className="primary" disabled={readOnly}>{t("admin.createTeam")}</button></div></form></EditorDrawer><div className="admin-grid">{visibleTeams.map((entity) => <div className={highlights[entity.document.id] ? "recently-changed" : ""} key={entity.document.id}><TeamEditor {...{ api, draft, entity, fingerprint, readOnly, t, people: activePeople, mutate, remove, confirmDelete }} /></div>)}</div></section></div>}
...
function CalendarEditor(props: EditorProps & { readonly locale: Locale }) {
  ...
  const submit = async (form: HTMLFormElement) => { const data = new FormData(form); return await mutate(async () => await api.updateEntity(draft.draft_id, "calendars", entity, fingerprint, { ...entity.document, name: String(data.get("name")), working_weekdays: data.getAll("weekdays").map(Number), holidays: split(data.get("holidays")) })) !== null; };
  return <article className="admin-card admin-card-summary"><strong>{name}</strong><div aria-label={t("admin.weekPreview")} className="calendar-week-preview">{[1, 2, 3, 4, 5, 6, 7].map((day) => <span className={weekdays.includes(day) ? "working" : "off"} key={day}>{t(`admin.day${day}` as MessageKey)}</span>)}</div><div className="calendar-exceptions"><span>{t("admin.nextHolidays")}</span>{holidays.length === 0 ? <small>{t("admin.noHolidays")}</small> : holidays.map((date) => <time dateTime={date} key={date}>{formatDateOnly(locale, date)}</time>)}</div><button className="editor-trigger" onClick={() => setOpen(true)} type="button">{t("admin.editCalendar")}</button><EditorDrawer closeLabel={t("core.closeEditor")} onClose={() => setOpen(false)} open={open} title={`${t("admin.editCalendar")}: ${name}`}><form className="editor-drawer-form" onSubmit={(event) => event.preventDefault()}><label>{t("core.name")}<input name="name" aria-label={`${t("core.name")} ${name}`} defaultValue={name} /></label><WeekdayChecks selected={weekdays} t={t} /><label>{t("admin.holidays")}<input name="holidays" aria-label={`${t("admin.holidays")} ${name}`} defaultValue={strings(entity.document, "holidays").join(",")} /></label><ActionButtons disabled={readOnly} t={t} close={() => setOpen(false)} save={submit} archive={async () => await mutate(async () => await api.archiveEntity(draft.draft_id, "calendars", entity, fingerprint)) !== null} remove={async () => confirmDelete(name) && await remove(async () => await api.deleteEntity(draft.draft_id, "calendars", entity, fingerprint))} /></form></EditorDrawer></article>;
}

function PersonEditor(props: EditorProps & { calendars: readonly EntityResult[] }) {
  const { api, draft, entity, fingerprint, readOnly, t, calendars, mutate, remove, confirmDelete } = props;
  const [open, setOpen] = useState(false);
  const name = text(entity.document, "name");
  const calendar = calendars.find((item) => item.document.id === text(entity.document, "calendar"));
  const calendarName = calendar === undefined ? "" : text(calendar.document, "name");
  const update = async (form: HTMLFormElement) => { const data = new FormData(form); const email = String(data.get("email")); return await mutate(async () => await api.updateEntity(draft.draft_id, "people", entity, fingerprint, { ...entity.document, name: String(data.get("name")), email: email || undefined, weekly_capacity_hours: Number(data.get("capacity")), calendar: String(data.get("calendar")) })) !== null; };
  return <article className="admin-card admin-card-summary"><strong>{name}</strong>{text(entity.document, "email") !== "" && <span>{text(entity.document, "email")}</span>}<span>{t("admin.capacitySummary", { count: number(entity.document, "weekly_capacity_hours"), calendar: calendarName })}</span><button className="editor-trigger" onClick={() => setOpen(true)} type="button">{t("admin.editPerson")}</button><EditorDrawer closeLabel={t("core.closeEditor")} onClose={() => setOpen(false)} open={open} title={`${t("admin.editPerson")}: ${name}`}><form className="editor-drawer-form" onSubmit={(event) => event.preventDefault()}><label>{t("core.name")}<input name="name" aria-label={`${t("core.name")} ${name}`} defaultValue={name} /></label><label>{t("admin.email")}<input name="email" aria-label={`${t("admin.email")} ${name}`} defaultValue={text(entity.document, "email")} /></label><label>{t("admin.capacity")}<input name="capacity" type="number" min="0" step="0.25" aria-label={`${t("admin.capacity")} ${name}`} defaultValue={number(entity.document, "weekly_capacity_hours")} /></label><label>{t("admin.calendar")}<select name="calendar" aria-label={`${t("admin.calendar")} ${name}`} defaultValue={text(entity.document, "calendar")}>{calendars.map((item) => <option key={item.document.id} value={item.document.id}>{text(item.document, "name")}</option>)}</select></label><ActionButtons disabled={readOnly} t={t} close={() => setOpen(false)} save={update} archive={async () => await mutate(async () => await api.archiveEntity(draft.draft_id, "people", entity, fingerprint)) !== null} remove={async () => confirmDelete(name) && await remove(async () => await api.deleteEntity(draft.draft_id, "people", entity, fingerprint))} /></form></EditorDrawer></article>;
}
...
function TeamEditor(props: EditorProps & { people: readonly EntityResult[] }) {
  const { api, draft, entity, fingerprint, readOnly, t, people, mutate, remove, confirmDelete } = props;
  const [open, setOpen] = useState(false);
  const name = text(entity.document, "name");
  const selected = strings(entity.document, "members");
  const memberNames = selected.map((id) => people.find((person) => person.document.id === id)).filter((person): person is EntityResult => person !== undefined).map((person) => text(person.document, "name"));
...
function ConfigEditor({ api, draft, entity, kind, listKey, title, readOnly, t, mutate }: { readonly api: GitPmApi; readonly draft: DraftStatus; readonly entity: EntityResult; readonly kind: "statuses" | "issue-types"; readonly listKey: "statuses" | "issue_types"; readonly title: string; readonly readOnly: boolean; readonly t: (key: MessageKey, values?: Readonly<Record<string, string | number>>) => string; readonly mutate: (operation: () => Promise<EntityResult>) => Promise<EntityResult | null> }) {
  ...
  const persist = async (next: readonly ConfigValue[]) => {
    const previous = values; setValues([...next]); setBusy(true);
    const result = await mutate(async () => await api.updateConfiguration(draft.draft_id, kind, entity, entity.draft_fingerprint, { ...entity.document, [listKey]: next }));
