...
import { scheduleText, scheduleEffort, scheduleStart, scheduleFinish } from "./schedules.js";
...
function task(entity: EntityResult, track?: string): WorkloadTask {
  const document = entity.document;
  return { id: document.id, title: text(document, "title") ?? document.id, lifecycle: document.lifecycle, estimate_hours: scheduleEffort(document, track), start: scheduleStart(document, track), due: scheduleFinish(document, track), assignees: strings(document, "assignees") };
}
...
export function WorkloadWorkspace({ api, draft, locale, onNavigate = () => undefined }: { readonly api: GitPmApi; readonly draft: DraftStatus; readonly locale: Locale; readonly onNavigate?: WorkspaceNavigate }) {
  ...
  const workloadTrackByProject = useMemo(() => new Map(projects.map((item) => [item.document.id, ((item.document.planning as { readonly workload_track?: string } | undefined)?.workload_track) ?? "plan"])), [projects]);
  const filterMilestones = milestones.filter((item) => projectFilter === "" || item.document.project === projectFilter);
  const report = useMemo(() => calculateWorkload(filteredTasks.map((entity) => task(entity, workloadTrackByProject.get(String(entity.document.project ?? "")))), people.map(person), calendars.map(calendar)), [filteredTasks, people, calendars, workloadTrackByProject]);
