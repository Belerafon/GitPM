...
import { scheduleStart, scheduleFinish, scheduleEffort, ScheduleResolver, scheduleTracksConfig } from "./schedules.js";
...
function task(entity: EntityResult, track: string): WorkloadTask {
  ...
  return { id: document.id, title: text(document, "title") ?? document.id, lifecycle: document.lifecycle, estimate_hours: scheduleEffort(document, track), start: scheduleStart(document, track), finish: scheduleFinish(document, track), assignees: strings(document, "assignees") };
}
...
export function WorkloadWorkspace({ api, draft, locale, onNavigate = () => undefined }: { readonly api: GitPmApi; readonly draft: DraftStatus; readonly locale: Locale; readonly onNavigate?: WorkspaceNavigate }) {
  ...
  const workloadTrackByProject = useMemo(() => new Map(projects.map((item) => [item.document.id, scheduling.workloadTrack(item.document.planning)])), [projects, scheduling]);
