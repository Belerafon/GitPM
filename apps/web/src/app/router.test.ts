...
describe("app route model", () => {
  it.each([
    "/workspaces", "/projects", "/tasks", "/board", "/people", "/calendars", "/settings", "/workload", "/vacations", "/gantt", "/changes", "/files", "/history",
  ...
  it("maps contextual workspace destinations to canonical routes", () => {
    ...
    expect(serializeAppRoute(routeForDestination("vacations"))).toBe("/vacations");
  });
