...
export function SectionTabs({ active, ariaLabel, items, onNavigate, t }: {
  ...
  readonly onNavigate: (destination: WorkspaceDestination | "workspaces", selection?: WorkspaceSelection) => void;
  readonly t: (key: MessageKey) => string;
}) {
  return <nav aria-label={ariaLabel} className="section-tabs">
    {items.map((item) => <button aria-current={active === item.label ? "page" : undefined} className={active === item.label ? "active" : ""} key={item.label} onClick={() => onNavigate(item.destination, item.selection)} type="button">{t(item.label)}</button>)}
  </nav>;
}
