...
describe("scripted agent CLI", () => {
  it("runs external edit through format, validate, semantic diff, scope/delete guards, commit-all, push and MR", async () => {
    ...
    await atomicWriteDomainFile(draft.worktree_path, projectFile, (await readFile(path.join(draft.worktree_path, ...projectFile.split("/")), "utf8")).replace("name: GitPM launch", "name: Agent CLI delivery"));
