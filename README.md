...
remote один раз на test file и выдают каждому тесту отдельную копию. Playwright запускает
...
одним worker для устойчивости при параллельных проверках репозиториев; на свободной машине
