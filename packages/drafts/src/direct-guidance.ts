...
- do not commit unless the user explicitly requests a commit; when requested, commit the
  complete change atomically through GitPM;
...
- Explicit workflow over convenience: validate, review, wait for an explicit user request before
  committing all, and publish deliberately.
...
\`.gitpm/issue-types.yaml\`. Global entity directories are \`people/\`, \`teams/\`,
\`calendars/\`, and \`availability/\`. A Project occupies \`projects/<P-id>/\` and contains \`project.yaml\`, plus
...
Never write, rename, or delete data below \`.gitpm/\`, \`people/\`, \`teams/\`, \`calendars/\`,
\`availability/\`, or \`projects/\` with an editor, shell redirection, scripts, filesystem tools, raw Git, an MCP server,
...
- \`gitpm entity list --type <type> [--project <id>]\` lists entities of a type, optionally
  filtered by Project.
- \`gitpm entity show --type <type> --id <entity-id>\` returns a single entity document.
- \`gitpm entity delete --type <type> --id <entity-id> [--unlink-references|--cascade-references] [--dry-run]
  [--allow-delete] [--project <id>]\` removes an entity file. Task deletion cascades to that
  task's comments. \`--dry-run\` previews the reference impact without writing.
...
- \`gitpm comment create --project <id> --task <id> (--body <text> | --file <path>)\` adds a
  Markdown comment with optional \`@[Name](person:U-...)\` mentions.
