...
Конфигурационные документы без собственного `id` также входят в `created`, `updated` и
`deleted`: у таких элементов `id` отсутствует, а `schema`, `path` и изменения полей остаются
обязательными. Элементы упорядоченных конфигурационных списков со `slug` сравниваются по этому
стабильному ключу, поэтому, например, переименование варианта расписания отображается полем
`tracks.<slug>.title`, а не заменой всего массива.
