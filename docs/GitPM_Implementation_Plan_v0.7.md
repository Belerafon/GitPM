...
Статус: архитектурный baseline v0.1 с принятыми post-release дополнениями
...
- Project, Task, Milestone, Person, Team, Calendar, Saved View и Comment;
...
- работа агента через общий CLI в `direct` или изолированном `worktree` mode;
...
Исключение для недоменных материалов Project — опциональное плоское хранилище
`projects/<project-id>/files/`. В нём обычные файлы любых расширений хранятся как непрозрачное
Git-содержимое и не разбираются как domain YAML; manifest или отдельная сущность со списком файлов
не создаются. Имена должны быть Windows-совместимыми и уникальными внутри Project без учёта
...
Project, Task, Milestone, Person, Team, Calendar и Saved View содержат:

- `schema`: строка вида `gitpm/<type>@1`;
- `id`: immutable short ID;
- `lifecycle`: `active` или `archived`.

Comment использует отдельный immutable audit contract: `state` равен `active`
или `deleted`; удаление оставляет tombstone metadata без исходного текста.
...
содержит `body_markdown`; edited comment получает `updated_at`. Mention хранит
Person ID и timestamp. Deleted comment удаляет body/mentions и сохраняет
...
- `allowed_top_level_directories` — имена дополнительных non-domain каталогов
...
- Project plan содержит отдельный архив с количеством архивных Milestone и Task; архивный
  Milestone сохраняет исходную иерархию всех связанных Task и не превращает их в «задачи без этапа»;
...
Этот раздел относится к `worktree` mode. Перед каждым созданием draft server под repository-wide lock выполняет fetch. Новый draft создается только от точного commit `origin/<default_branch>`, который записывается как `base_commit`.
...
В `worktree` mode draft metadata содержит:
...
Для Milestone пользователь явно выбирает одиночный переход либо атомарный переход вместе со
всеми связанными Task соответствующего lifecycle. При восстановлении отдельной Task с архивным
Milestone доступен атомарный переход Task + Milestone. Все составные переходы защищены одним
optimistic fingerprint, валидируют итоговое состояние целиком и полностью откатываются при ошибке.
...
gitpm commit --all -m "..."
gitpm push
# Только worktree mode:
gitpm mr create --draft <id> ...
```
...
GitPM запускается локально как Node.js process с системным Git или из
...
`compose.yaml` предназначен для доверенного local profile, а optional
`compose.server.yaml` добавляет bind IP, healthcheck, GitLab OAuth configuration
и persistent application config. Public exposure требует отдельного reverse
proxy/TLS и deployment review. Server configuration задается config file,
environment и mounted secrets. Backup не входит в продукт.
...
### 27.4.2. Project files в Changes

Changes API выводит канонические Project files отдельным обязательным read model, производным от
реального Git status и содержимого рабочей копии. Базы, manifest и sidecar нет. Бинарные изменения
имеют семантику added/replaced/renamed/deleted; текст сохраняет обычный безопасно ограниченный Git
diff. Text/binary определяется по ограниченному образцу байтов, не по расширению.

Unstaged rename сообщается только при явном Git rename либо доказанной единственной exact-byte паре
внутри одного Project. Неустановленная связь остаётся delete + add, а межпроектный перенос никогда
не расширяет scope и не объединяется. Неканонический внешний путь остаётся unclassified и
исправляется через validation/технический менеджер.
