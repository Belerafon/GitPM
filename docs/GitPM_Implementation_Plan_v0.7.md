...
GitPM является Git-first системой управления проектами и задачами. Пользователь или агент изменяет YAML-файлы в отдельной ветке и `git worktree`, просматривает file diff и semantic diff, выполняет validation, commit, push и создает GitLab Merge Request.
...
У каждой доменной сущности есть один immutable ID с типовым префиксом и ULID:
...
- GitPM генерирует collision-resistant ULID и проверяет уникальность в текущем repository state;
...
- `default_branch`;
- `default_calendar`;
- разрешенные top-level служебные файлы;
- UI polling interval в диапазоне 2-10 секунд.

GitLab URL, project ID, repository URL и OAuth secret относятся к server configuration и не редактируются через UI.

`.gitpm/statuses.yaml` и `.gitpm/issue-types.yaml` содержат упорядоченные списки конфигурационных значений с immutable slug, title, color token и `active`. Эти записи не являются ULID-сущностями. Maintainer может редактировать их через repository settings UI.
