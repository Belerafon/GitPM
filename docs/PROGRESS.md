...
- добавлен защищённый файловый менеджер рабочего дерева: навигация по папкам,
  предпросмотр текстовых файлов, скачивание любых обычных файлов, а также создание папок, загрузка файлов,
  переименование, перемещение и удаление через optimistic-fingerprint мутации; файлы с компьютера можно
  загружать перетаскиванием, а существующие файлы — перетаскивать в папки;
