...
Person хранит обязательное поле имени `name` и необязательные `family_name` и `middle_name`.
Отображаемая подпись вычисляется, а не хранится отдельно. `display_name_format` принимает `full`
или `family-initials`; при его отсутствии используется `default_person_name_format` из
`.gitpm/repository.yaml` (по умолчанию `full`). Полный формат пропускает пустые части и выводит
`family_name name middle_name`. Формат инициалов выводит фамилию и инициалы имени/отчества;
если фамилия не заполнена, имеющиеся части выводятся полностью. Поиск Person учитывает все три поля.
...
Штатное переименование с обновлением ссылок заменяет каждый exact token старого имени на
канонический token нового имени во всех перечисленных полях одного Project. Штатное отвязывание
перед удалением заменяет token decoded именем файла как обычным видимым текстом. Файл, YAML-тексты
и полная repository validation входят в одну optimistic mutation: при любой ошибке восстанавливаются
исходные байты всех затронутых YAML-файлов и исходное файловое имя или содержимое. Осознанное
сохранение старых ссылок после rename допустимо, но результат сообщает оставшееся количество
сломанных ссылок; неизвестное или непроверенное состояние не эквивалентно нулю.
...
Штатная замена выбранного файла новым локальным файлом является одной optimistic mutation. При
новом имени она одновременно заменяет байты, меняет каноническое имя и переписывает все exact
file tokens текущего Project; при прежнем имени сохраняет ссылки и заменяет только содержимое.
Конфликт целевого имени не перезаписывается. Перед успехом выполняется полная repository validation,
финальный пересчёт ссылок при прежнем имени либо проверка отсутствия ссылок на старое имя при новом.
Ошибка восстанавливает точные исходные байты файла и всех переписанных YAML best-effort, не
перезаписывая независимо изменившийся внешний файл.
