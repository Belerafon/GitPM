...
не валидируются и не попадают в GitPM-снапшоты. Их назначение — хранить
пользовательские артефакты (например, `uploads/` для исходных документов,
которые агент читает, но не коммитит). Чтобы такие файлы не попали в историю
Git, корневой `.gitignore` должен игнорировать соответствующие пути.
